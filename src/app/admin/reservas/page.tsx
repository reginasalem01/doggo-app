...
const STATUS_COLOR: Record<string, string> = {
  pending: 'bg-doggo-yellow/20 text-doggo-yellow',
...
export default async function AdminReservasPage() {
  ...
  return (
    <div className="h-full flex flex-col bg-doggo-dark p-4 gap-3 overflow-hidden">

      {/* Header */}
      <div className="flex items-center gap-3 shrink-0">
        <h1 className="text-white text-xl font-black">Reservas</h1>
        {pendientes.length > 0 && (
      ...
      <div className="flex gap-3 flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <p className="text-doggo-yellow text-xs font-bold uppercase tracking-wide mb-2 shrink-0">
            Requieren confirmación ({pendientes.length})
          </p>
          {pendientes.length === 0 ? (
            <div className="bg-doggo-dark2 rounded-2xl p-5 text-center flex-1 flex flex-col items-center justify-center">
              <p className="text-2xl mb-1">✅</p>
              <p className="text-white font-bold text-sm">Sin pendientes</p>
            </div>
          ) : (
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          {resto.length === 0 ? (
            <div className="bg-doggo-dark2 rounded-2xl p-5 text-center flex-1 flex items-center justify-center">
    ...
    <div className={`bg-doggo-dark2 rounded-2xl p-4 ${showActions ? 'border border-doggo-yellow/30' : ''}`}>
      <div className="flex items-start justify-between gap-3 mb-1">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-400 text-xs mt-0.5">{r.customer_phone as string}</p>
